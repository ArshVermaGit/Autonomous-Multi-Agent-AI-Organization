...
class BackendAgent(BaseAgent):
    """
    Backend Engineer Agent for code generation.
    Includes self-fix loop (lint → fix → commit).
    """

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.ROLE = "Engineer_Backend"
                    ...
                    if isinstance(response_json, dict):
                        ...
                        for path, code in response_json.items():
                            if path.endswith(".py"):
                                ...
                                if not safe:
                                    logger.error(
                                        "DANGEROUS CODE BLOCKED", file=path, reason=msg
                                    )
                                    raise RuntimeError(
                                        f"Security Policy Violation in {path}: {msg}"
                                    )

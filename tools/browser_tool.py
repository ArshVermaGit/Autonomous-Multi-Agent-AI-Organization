...
class BrowserTool(BaseTool):
    NAME = "browser_tool"
    DESCRIPTION = (
        "Automates browser UI workflows using Amazon Nova Act to extract text, test UIs, and navigate."
    )
    TIMEOUT_S = 300
    ...
    async def run(self, action: str, url: Optional[str] = None, prompt: Optional[str] = None, **kwargs) -> ToolResult:
        """
        Main entrypoint.
        ...
        if action != "nova_act" and not prompt:
             return ToolResult(success=False, output="Must provide a 'prompt' for Nova Act.", error="Missing prompt")
        ...
        try:
            ...
            logger.warning("amazon-nova package not installed, falling back to mock Nova Act implementation")
            return self._mock_act(prompt, url)
        except Exception as e:
            return ToolResult(success=False, output="", error=f"Nova Act failed: {str(e)}")

...
class CollaborationTool(BaseTool):
    ...
    async def run(self, action: str, agent_name: str, target_agent: Optional[str] = None, message: Optional[str] = None, **kwargs) -> ToolResult:
        """
        Main entrypoint.
        ...
        if not fn:
            return ToolResult(
                success=False, output="", error=f"Unknown collaboration action: {action}"
            )
            
        logger.info("Executing collaboration action", action=action, agent=agent_name)
        return await fn(agent_name=agent_name, target_agent=target_agent, message=message, **kwargs)

    async def _post_message(self, agent_name: str, target_agent: Optional[str], message: str, **kwargs) -> ToolResult:
        if not message:
            return ToolResult(success=False, output="", error="Must provide a 'message' to post.")
    ...
    async def _read_all(self, agent_name: str, **kwargs) -> ToolResult:
        ...
        if not board:
            return ToolResult(success=True, output="The collaboration board is currently empty.")
            
        formatted_messages = []
        for msg in board:
            # Format: [10:30:15] Backend -> CTO: "I have finished the DB schema."
            t = time.strftime('%H:%M:%S', time.localtime(msg["timestamp"]))
            formatted_messages.append(f"[{t}] {msg['from_agent']} -> {msg['to_agent']}:\n{msg['message']}\n")

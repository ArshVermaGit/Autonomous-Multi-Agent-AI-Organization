...
pub fn task_type_to_vector(task_type: &str, context: &str) -> Vec<f64> {
    ...
    // ── [0] Strategy ─────────────────────────────────────────────────────────
    if contains_any(&t, &["strategy", "plan", "vision", "market", "mvp", "business"]) { v[0] = 0.9; }
    if contains_any(&c, &["strategy", "market", "business"]) { v[0] = v[0].max(0.5); }

    // ── [1] Architecture ─────────────────────────────────────────────────────
    if contains_any(&t, &["architect", "design", "schema", "tech_stack", "system"]) { v[1] = 0.9; }
    if contains_any(&c, &["architect", "api", "database"]) { v[1] = v[1].max(0.5); }

    // ── [2] Backend Code ─────────────────────────────────────────────────────
    if contains_any(&t, &["backend", "api", "fastapi", "database", "orm", "auth"]) { v[2] = 0.9; }
    if contains_any(&c, &["python", "fastapi", "sqlalchemy", "postgres"]) { v[2] = v[2].max(0.6); }

    // ── [3] Frontend Code ─────────────────────────────────────────────────────
    if contains_any(&t, &["frontend", "ui", "react", "nextjs", "component", "page"]) { v[3] = 0.9; }
    if contains_any(&c, &["typescript", "next.js", "tailwind", "css"]) { v[3] = v[3].max(0.6); }

    // ── [4] Testing ───────────────────────────────────────────────────────────
    if contains_any(&t, &["test", "qa", "security_scan", "coverage", "validation"]) { v[4] = 0.9; }
    if contains_any(&c, &["pytest", "unittest", "bandit", "security"]) { v[4] = v[4].max(0.6); }

    // ── [5] DevOps ────────────────────────────────────────────────────────────
    if contains_any(&t, &["devops", "deploy", "terraform", "docker", "kubernetes", "ci_cd"]) { v[5] = 0.9; }
    if contains_any(&c, &["aws", "ecs", "k8s", "helm", "infra"]) { v[5] = v[5].max(0.6); }

    // ── [6] Cost Optimization ─────────────────────────────────────────────────
    if contains_any(&t, &["cost", "finance", "budget", "price", "optimization"]) { v[6] = 0.9; }
    if contains_any(&c, &["cost", "budget", "spend", "usd", "savings"]) { v[6] = v[6].max(0.5); }

    // ── [7] Security ──────────────────────────────────────────────────────────
    if contains_any(&t, &["security", "iam", "auth", "permission", "audit"]) { v[7] = 0.9; }
    if contains_any(&c, &["vulnerability", "injection", "xss", "csrf", "secret"]) { v[7] = v[7].max(0.5); }
...
pub fn direct_expert_for_task_type(task_type: &str) -> Option<&'static str> {
    match task_type.to_lowercase().as_str() {
        ...
        "backend_development" | "api_implementation" | "database_setup" | "auth_implementation" => Some("Engineer_Backend"),
        "frontend_development" | "ui_implementation" | "component_development" => Some("Engineer_Frontend"),
        "qa_testing" | "security_scan" | "performance_testing" | "test_coverage" => Some("QA"),
        "infrastructure_setup" | "ci_cd_pipeline" | "deployment" | "monitoring_setup" => Some("DevOps"),
...
mod tests {
    ...
    fn test_unknown_task_uniform() {
        ...
        assert!(v.iter().all(|&x| (x - 0.125).abs() < 1e-9), "unknown → uniform");
    }
